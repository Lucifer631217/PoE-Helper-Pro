...
export const CheatsheetViewer = ({ images, onClose, hotkeyLabel }) => {
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="poe-viewer fixed inset-0 z-[100] flex flex-col overflow-hidden"
            onWheel={handleWheel}
        >
            <div className="poe-viewer-toolbar h-8 flex items-center justify-between px-3 shrink-0 select-none border-b" style={{ WebkitAppRegion: 'drag' }}>
                <div className="flex items-center gap-3" style={{ WebkitAppRegion: 'no-drag' }}>
                    <div className="poe-viewer-chip flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-bold border">
                        <kbd>{hotkeyLabel || 'F9'}</kbd> 隱藏
                    </div>
                    {srcs.length > 1 && (
                        <div className="flex items-center gap-1">
                            <button onClick={prevImg} className="poe-viewer-btn p-1 rounded transition-colors"><ChevronLeft size={12} /></button>
                            <span className="text-gray-400 text-[10px] font-bold min-w-[32px] text-center">{safeIdx + 1}/{srcs.length}</span>
                            <button onClick={nextImg} className="poe-viewer-btn p-1 rounded transition-colors"><ChevronRight size={12} /></button>
                        </div>
                    )}
                    <span className="text-gray-400 text-[10px] font-medium hidden sm:inline">縮放: {Math.round(scale * 100)}% · 拖曳平移</span>
                    <button onClick={() => setScale(1)} className="poe-viewer-btn text-[10px] px-1.5 py-0.5 rounded transition-colors">100%</button>
                    <button onClick={() => setScale(s => s * 0.5)} className="poe-viewer-btn text-[10px] px-1.5 py-0.5 rounded transition-colors">50%</button>
                </div>
                <button onClick={onClose} style={{ WebkitAppRegion: 'no-drag' }} className="poe-viewer-close p-1.5 rounded-md transition-colors">
                    <X size={14} />
                </button>
            </div>

            <div className="poe-viewer-stage flex-1 relative overflow-hidden flex items-center justify-center" style={{ WebkitAppRegion: 'no-drag' }}>
